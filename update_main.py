...
text = text.replace("    def load_dataset(self, file_path, sheet_name=None):", switch_code + "\n    def load_dataset(self, file_path, sheet_name=None):")

# Modify load_dataset to return dataset_id
load_dataset_end = """            preview_data = json.loads(preview_df.to_json(orient="records", date_format="iso"))
...
text = text.replace("""            preview_data = json.loads(preview_df.to_json(orient="records", date_format="iso"))
            ...
            return {
                "success": True,
                "row_count": len(df),""", load_dataset_end)
